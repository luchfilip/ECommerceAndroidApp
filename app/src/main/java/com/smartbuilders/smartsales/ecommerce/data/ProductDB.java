...
import com.smartbuilders.smartsales.ecommerce.utils.ProductDBUtils;
...
public class ProductDB {
    ...
    public ArrayList<Product> getRelatedShoppingProductsByProductId(int productId, Integer limit){
        ...
        try {
            StringBuilder sql = new StringBuilder("SELECT DISTINCT P.PRODUCT_ID, P.SUBCATEGORY_ID, P.BRAND_ID, P.NAME, P.DESCRIPTION, P.PURPOSE, ")
                    ...
                    .append(mShowProductsWithoutAvailability ? " LEFT " : " INNER ").append(" JOIN PRODUCT_PRICE_AVAILABILITY PA ON PA.PRICE_LIST_ID = COALESCE((SELECT PRICE_LIST_ID FROM BUSINESS_PARTNER WHERE BUSINESS_PARTNER_ID=").append(Utils.getAppCurrentBusinessPartnerId(mContext, mUser)).append(" AND IS_ACTIVE='Y'),0) AND PA.PRODUCT_ID = P.PRODUCT_ID AND PA.IS_ACTIVE = 'Y' AND PA.AVAILABILITY > 0 ")
    ...
    public ArrayList<ProductSearchResult> getLightProductsByName(String name){
        ...
        try {
            StringBuilder sql = new StringBuilder("SELECT P.PRODUCT_ID,P.SUBCATEGORY_ID,UPPER(P.NAME),P.INTERNAL_CODE,S.NAME ")
                    ...
                    .append(" INNER JOIN SUBCATEGORY S ON S.SUBCATEGORY_ID = P.SUBCATEGORY_ID AND S.IS_ACTIVE = 'Y' ")
                    .append(" INNER JOIN CATEGORY C ON C.CATEGORY_ID = S.CATEGORY_ID AND C.IS_ACTIVE = 'Y' ")
                    .append(mShowProductsWithoutAvailability ? " LEFT " : " INNER ").append(" JOIN PRODUCT_PRICE_AVAILABILITY PA ON PA.PRICE_LIST_ID = COALESCE((SELECT PRICE_LIST_ID FROM BUSINESS_PARTNER WHERE BUSINESS_PARTNER_ID=").append(Utils.getAppCurrentBusinessPartnerId(mContext, mUser)).append(" AND IS_ACTIVE='Y'),0) ")
                    .append(" AND PA.PRODUCT_ID = P.PRODUCT_ID AND PA.IS_ACTIVE = 'Y' AND PA.AVAILABILITY > 0 ");
    ...
    public Product getProductById(int id){
        ...
        try {
            StringBuilder sql = new StringBuilder("SELECT DISTINCT P.PRODUCT_ID, P.SUBCATEGORY_ID, P.BRAND_ID, P.NAME, P.DESCRIPTION, P.PURPOSE, ")
                    ...
                    .append(" INNER JOIN SUBCATEGORY S ON S.SUBCATEGORY_ID = P.SUBCATEGORY_ID AND S.IS_ACTIVE = 'Y' ")
                    .append(" INNER JOIN CATEGORY C ON C.CATEGORY_ID = S.CATEGORY_ID AND C.IS_ACTIVE = 'Y' ")
                    .append(" LEFT JOIN PRODUCT_PRICE_AVAILABILITY PA ON PA.PRICE_LIST_ID = COALESCE((SELECT PRICE_LIST_ID FROM BUSINESS_PARTNER WHERE BUSINESS_PARTNER_ID=").append(Utils.getAppCurrentBusinessPartnerId(mContext, mUser)).append(" AND IS_ACTIVE='Y'),0) AND PA.PRODUCT_ID = P.PRODUCT_ID AND PA.IS_ACTIVE = 'Y' ")
                    .append(" LEFT JOIN CURRENCY CU ON CU.CURRENCY_ID = PA.CURRENCY_ID AND CU.IS_ACTIVE = 'Y' ")
                    .append(" LEFT JOIN PRODUCT_TAX PT ON PT.PRODUCT_TAX_ID = P.PRODUCT_TAX_ID AND PT.IS_ACTIVE = 'Y' ")
                    .append(" LEFT JOIN PRODUCT_IMAGE PI ON PI.PRODUCT_ID = P.PRODUCT_ID AND PI.PRIORITY = 1 AND PI.IS_ACTIVE = 'Y' ")
                    .append(" LEFT JOIN PRODUCT_RATING PR ON PR.PRODUCT_ID = P.PRODUCT_ID AND PR.IS_ACTIVE = 'Y' ")
     ...
     * @return
     */
    public HashMap<Integer, Product> getProductsByIds(String productsIds){
        Cursor c = null;
        ...
        try {
            StringBuilder sql = new StringBuilder("SELECT DISTINCT P.PRODUCT_ID, P.SUBCATEGORY_ID, P.BRAND_ID, P.NAME, P.DESCRIPTION, P.PURPOSE, ")
                    ...
                    .append(" LEFT JOIN PRODUCT_RATING PR ON PR.PRODUCT_ID = P.PRODUCT_ID AND PR.IS_ACTIVE = 'Y' ")
                    .append(" LEFT JOIN ECOMMERCE_ORDER_LINE OL ON OL.PRODUCT_ID = P.PRODUCT_ID AND OL.USER_ID = ").append(mUser.getServerUserId()).append(" AND OL.DOC_TYPE = '").append(OrderLineDB.WISH_LIST_DOC_TYPE).append("' AND OL.IS_ACTIVE = 'Y' ")
                    .append(" WHERE P.PRODUCT_ID IN (").append(productsIds).append(") AND P.IS_ACTIVE = 'Y'");
            c = mContext.getContentResolver()
                    .query(DataBaseContentProvider.INTERNAL_DB_URI.buildUpon()
                                    .appendQueryParameter(DataBaseContentProvider.KEY_USER_ID, mUser.getUserId()).build(),
                            null, sql.toString(), null, null);
            if (c!=null) {
                while (c.moveToNext()){
                    ...
                    fillFullProductInfoFromCursor(product, c);
