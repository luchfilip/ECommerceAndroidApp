...
public class OrderTrackingDB {
    ...
    /**
     * Devuelve un listado de los orderTracking que no poseen notificacion
     * @return
     */
    public ArrayList<OrderTracking> getOrderTrackingWithoutNotification() {
        //try {
        //    mContext.getContentResolver().update(DataBaseContentProvider.INTERNAL_DB_URI.buildUpon()
        //            .appendQueryParameter(DataBaseContentProvider.KEY_USER_ID, mUser.getUserId()).build(),
        //            null,
        //            "INSERT INTO ORDER_TRACKING (ORDER_TRACKING_ID, USER_ID, ECOMMERCE_ORDER_ID, ORDER_TRACKING_STATE_ID, DETAILS, CREATE_TIME) VALUES (1, 238, 46, 1, '', '2016-11-23 14:30:00') ", null);
        //} catch (Exception e) {
        //    e.printStackTrace();
        //}
        ArrayList<OrderTracking> orderTrackings = new ArrayList<>();
        ...
        try {
            c = mContext.getContentResolver().query(DataBaseContentProvider.INTERNAL_DB_URI.buildUpon()
                    ...
                    "SELECT OT.ECOMMERCE_ORDER_ID, OT.ORDER_TRACKING_STATE_ID, OT.DETAILS, OT.CREATE_TIME, " +
                        ...
                        " OTS.IS_ALWAYS_VISIBLE " +
                    " FROM ORDER_TRACKING_STATE  OTS " +
                        " LEFT JOIN ORDER_TRACKING OT ON OT.ORDER_TRACKING_STATE_ID = OTS.ORDER_TRACKING_STATE_ID AND OT.USER_ID = ? AND OT.IS_ACTIVE = ? " +
                        " LEFT JOIN NOTIFICATION_HISTORY N ON N.RELATED_ID = OT.ORDER_TRACKING_ID AND N.TYPE = ? " +
                    " WHERE OTS.IS_ACTIVE = ? AND N.RELATED_ID IS NULL " +
                    " ORDER BY OT.CREATE_TIME DESC, OTS.PRIORITY ASC",
                    new String[]{String.valueOf(mUser.getServerUserId()), "Y", "Y",
                            String.valueOf(NotificationHistory.TYPE_NEW_ORDER_TRACKING)},
                    null);
            if(c!=null){
                while(c.moveToNext()){
                    OrderTracking orderTracking = new OrderTracking();
                    ...
                    orderTrackingState.setIconResName(c.getString(5));
                    orderTrackingState.setIconFileName(c.getString(6));
                    orderTrackingState.setBackground_R_Color(c.getString(7)==null ? -1 : c.getInt(6));
                    orderTrackingState.setBackground_G_Color(c.getString(8)==null ? -1 : c.getInt(7));
                    orderTrackingState.setBackground_B_Color(c.getString(9)==null ? -1 : c.getInt(8));
                    orderTrackingState.setBorder_R_Color(c.getString(10)==null ? -1 : c.getInt(9));
                    orderTrackingState.setBorder_G_Color(c.getString(11)==null ? -1 : c.getInt(10));
                    orderTrackingState.setBorder_B_Color(c.getString(12)==null ? -1 : c.getInt(11));
                    orderTrackingState.setTitle_R_Color(c.getString(13)==null ? -1 : c.getInt(12));
                    orderTrackingState.setTitle_G_Color(c.getString(14)==null ? -1 : c.getInt(13));
                    orderTrackingState.setTitle_B_Color(c.getString(15)==null ? -1 : c.getInt(14));
                    orderTrackingState.setIcon_R_Color(c.getString(16)==null ? -1 : c.getInt(15));
                    orderTrackingState.setIcon_G_Color(c.getString(17)==null ? -1 : c.getInt(16));
                    orderTrackingState.setIcon_B_Color(c.getString(18)==null ? -1 : c.getInt(17));
                    orderTrackingState.setAlwaysVisible(String.valueOf(c.getString(19)).equals("Y"));
                    orderTracking.setOrderTrackingState(orderTrackingState);
                    orderTrackings.add(orderTracking);
                }
