...
public class CurrencyDB {
    ...
    public Currency getActiveCurrencyById(int taxId) {
        ...
        try {
            c = mContext.getContentResolver().query(DataBaseContentProvider.INTERNAL_DB_URI, null,
                    "SELECT COUNTRY_NAME, CODE, UNICODE_DECIMAL, UNICODE_HEX FROM CURRENCY WHERE CURRENCY_ID=? AND IS_ACTIVE=?",
                    new String[]{String.valueOf(taxId), "Y"}, null);
            if(c!=null && c.moveToNext()){
                Currency currency = new Currency();
                currency.setId(taxId);
                currency.setCountryName(c.getString(0));
                currency.setInternationalCode(c.getString(1));
                currency.setUnicodeDecimal(c.getInt(2));
                currency.setUnicodeHex(c.getString(3));
                return currency;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public ArrayList<Currency> getActiveCurrenciesList() {
        ...
        try {
            c = mContext.getContentResolver().query(DataBaseContentProvider.INTERNAL_DB_URI, null,
                    "SELECT CURRENCY_ID, COUNTRY_NAME, CODE, UNICODE_DECIMAL, UNICODE_HEX FROM CURRENCY WHERE IS_ACTIVE=?",
                    new String[]{}, null);
            if(c!=null) {
                while (c.moveToNext()){
                    Currency currency = new Currency();
                    ...
                    currency.setUnicodeDecimal(c.getInt(3));
